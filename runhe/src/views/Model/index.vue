  ...
  <div class="chapter">
    ...
    <header class="chapter_search">
      <el-form :inline="true" ref="form" :model="form" label-width="60px" >
        ...
        <el-form-item>
          <el-button type="primary" @click="onAdd" class="chapter_add">新增模块</el-button>
        </el-form-item>
      </el-form> 
    </header>
    <el-table
    ...
    <el-table-column prop="id" label="模块编号" sortable></el-table-column>
...
export default {
  ...
  methods: {
    ...
    rerenderChapterData() {
      this.$http({
      url: `api/course?pk=${this.form.region}`,
      }).then(res => {
        let response = res.data;
        if(response.status == 'success') {
